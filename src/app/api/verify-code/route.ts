...
export async function POST(request: Request) {
  ...
  try {
    ...
    if(!result.success) {
      ...
      return Response.json({ success: false, message: codeErros?.length > 0 ? codeErros.join(", ") : "Invalid code" },{status: 400});
    }
    ...
    const isCodeExpired = new Date(user.verifyCodeExpireAt) < new Date();
    if(isCodeExpired) {
